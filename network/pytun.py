...
class Tunnel(object):
    ...
    def __init__(self, mode=None, pattern=None, auto_open=True, no_pi=True):
        ...
        if isinstance(self.mode, str):
            ...
            assert self.mode is not None, "%r is not a valid tunnel type." % (
                self.mode, )
    ...
    def open(self):
        ...
        logger.debug("Opening %s tunnel '%s'..." %
                     (self.mode_name.upper(), self.pattern, ))
        try:
            ret = fcntl.ioctl(self.fd, self.TUNSETIFF, struct.pack(
                "16sH", bytes(self.pattern, encoding="ascii"), self.mode | self.no_pi))
        except IOError as e:
            if e.errno == 1:
                logger.error("Cannot open a %s tunnel because the operation is not permitted." % (
    ...
    def set_ipv4(self, ip):
        ...
        ifreq = struct.pack('16sH2s4s8s', self.name,
                            socket.AF_INET, b'\x00'*2, bin_ip, b'\x00'*8)
        fcntl.ioctl(sock, self.SIOCSIFADDR, ifreq)
        ifreq = struct.pack('16sH', self.name, self.IFF_UP |
                            self.IFF_POINTOPOINT | self.IFF_RUNNING | self.IFF_MULTICAST)
