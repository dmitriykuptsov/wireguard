...
class WireGuardInitiatorPacket(WireGuardPacket):
    ...
    def ephimeral(self, e=None):
        if e:
            self.buffer[I_EPHIMERAL_OFFSET:I_EPHIMERAL_LENGTH +
                        I_EPHIMERAL_OFFSET] = e
        else:
...
class WireGuardResponderPacket(WireGuardPacket):
    ...
    def receiver(self, r=None):
        if r:
            self.buffer[R_RECEIVER_OFFSET:R_RECEIVER_OFFSET +
                        R_RECEIVER_LENGTH] = r
        else:
...
class WireGuardDataPacket(WireGuardPacket):
    ...
    def receiver(self, r=None):
        if r:
            self.buffer[D_RECEIVER_OFFSET:D_RECEIVER_OFFSET +
                        D_RECEIVER_LENGTH] = r
        else:
