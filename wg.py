    ...
    while True:
        ...
        try:
            while reading:
                ...
                if command == "status":
                    Spriv = crypto.curve25519.X25519PrivateKey.from_private_bytes(
                        b64decode(config.get(Config.KEY)))
    ...
    while True:
        ...
        try:
            if entry.state != Statemachine.States.ESTABLISHED and entry.rekey_timeout <= time():
                ...
                Epriv = crypto.curve25519.X25519PrivateKey.from_private_bytes(
                    os.urandom(32))
                ...
                logging.debug("Getting own EPUB %s" %
                              hexlify(packet.ephimeral()))
                h = crypto.digest.Digest()
                Hi = h.digest(Hi + packet.ephimeral())
                logging.debug("Hi HEX %s" % hexlify(Hi))
                (Ci, k) = crypto.digest.KDF.kdf2(Ci, Epriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Srpub)))
                aead = crypto.aead.AEAD(k, bytes([0x0] * 8))
                packet.static(aead.encrypt(Spub, Hi))
                aead = crypto.aead.AEAD(k, bytes([0x0] * 8))
                logging.debug("The public key to be transmitted is .... %s" % (b64encode(
                    aead.decrypt(packet.static()[:-16], Hi, packet.static()[-16:])).decode("ASCII")))
                h = crypto.digest.Digest()
                Hi = h.digest(Hi + packet.static())
                (Ci, k) = crypto.digest.KDF.kdf2(Ci, Spriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Srpub)))
                aead = crypto.aead.AEAD(k, bytes([0x0] * 8))
                ...
                h = crypto.digest.Digest()
                m = crypto.digest.MACDigest(
                    h.digest(crypto.constants.LABEL_MAC1 + Srpub))
    ...
    while True:
        ...
        try:
            if packet.type() == p.WIREGUARD_INITIATOR_TYPE:
                ...
                h = crypto.digest.Digest()
                m = crypto.digest.MACDigest(
                    h.digest(crypto.constants.LABEL_MAC1 + Spub))
                ...
                Hi = h.digest(Hi + Epub)
                (Ci, k) = crypto.digest.KDF.kdf2(Ci, Spriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Epub)))
                aead = crypto.aead.AEAD(k, bytes([0x0] * 8))
                Sipub = aead.decrypt(
                    packet.static()[:-16], Hi, packet.static()[-16:])
                ...
                if under_load:
                    ...
                    tau = m.digest(ip.encode("ASCII") +
                                   utils.misc.Math.int_to_bytes(int(port)))
                    packet = WireGuardCookiePacket()
                    packet.nonce(os.urandom(24))
                    packet.receiver(ii)
                    d = crypto.digest.Digest()
                    xaead = crypto.aead.xAEAD(
                        d.digest(crypto.constants.LABEL_COOKIE + Spub), packet.nonce())
                ...
                if time() - entry.cookie_timeout < 120:
                    ...
                    d = crypto.digest.Digest()
                    xaead = crypto.aead.xAEAD(
                        d.digest(crypto.constants.LABEL_COOKIE + Spub), entry.nonce)
                ...
                Hi = h.digest(Hi + packet.static())
                (Ci, k) = crypto.digest.KDF.kdf2(Ci, Spriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Sipub)))
                aead = crypto.aead.AEAD(k, bytes([0x0] * 8))
                timestamp = aead.decrypt(
                    packet.timestamp()[:-16], Hi, packet.timestamp()[-16:])
                ...
                Erpriv = crypto.curve25519.X25519PrivateKey.from_private_bytes(
                    os.urandom(32))
                ...
                Cr = crypto.digest.KDF.kdf1(Cr, Erpriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Epub)))
                Cr = crypto.digest.KDF.kdf1(Cr, Erpriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Sipub)))
                Q = bytes([0x0] * 32)
                ...
                h = crypto.digest.Digest()
                m = crypto.digest.MACDigest(
                    h.digest(crypto.constants.LABEL_MAC1 + Sipub))
                packet.mac1(m.digest(buffer))
                ...
                (Trecv, Tsend) = crypto.digest.KDF.kdf2(
                    Cr, crypto.constants.EMPTY)

                logging.debug(
                    "Sent reply to initiator packet.... to %s %s" % (ip, str(port)))
                ...
                Eipriv = crypto.curve25519.X25519PrivateKey.from_private_bytes(
                    entry.Epriv)
                Cr = crypto.digest.KDF.kdf1(Cr, Eipriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Erpub)))
                Cr = crypto.digest.KDF.kdf1(Cr, Spriv.exchange(
                    crypto.curve25519.X25519PublicKey.from_public_bytes(Erpub)))
                Q = bytes([0x0] * 32)
                (Cr, tau, k) = crypto.digest.KDF.kdf3(Cr, Q)
                h = crypto.digest.Digest()
                ...
                (Tsend, Trecv) = crypto.digest.KDF.kdf2(
                    Cr, crypto.constants.EMPTY)
                ...
                try:
                    data = aead.decrypt(
                        packet.data()[:-16], crypto.constants.EMPTY, packet.data()[-16:])
                except Exception as e:
                    logging.critical(e)
                    continue
...
def maintenance():
    ...
    while True:
        ...
        for entry in table.table:
            ...
            logging.debug(
                "State is missing (periodic task)... Running key exchange....")
            Srpub = entry.key
            h = crypto.digest.Digest()
            Ci = h.digest(crypto.constants.CONSTRUCTION)
            h = crypto.digest.Digest()
